...
export function WalletForm({ open, onOpenChange, wallet }: WalletFormProps) {
  ...
  const [walletType, setWalletType] = useState<WalletType>(wallet?.walletType ?? 'checking');
  const [currency, setCurrency] = useState<Currency>(wallet?.currency ?? 'INR');
  const [balance, setBalance] = useState(wallet ? formatAmount(wallet.balance) : '0.00');
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-2xl">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="mt-4 space-y-4 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            <label htmlFor="wallet-account" className="text-sm font-medium">
              Account Number
            </label>
            <Input
              id="wallet-account"
